...
function customModal({
  ...
  return new Promise((resolve) => {
    ...
    if (!isConfirm) {
      if (options && Array.isArray(options)) {
        ...
        const opts = options
          .map(
            (o) =>
              `<option value="${o.value}" ${String(o.value) === String(defaultValue) ? "selected" : ""}>${o.label}</option>`,
          )
          .join("");
    ...
    const confirmBtnText = isConfirm
      ? isDanger
        ? "Sim, Excluir"
        : "Confirmar"
      : "Confirmar";
...
function switchView(viewName) {
  ...
  const activeLink = document.querySelector(
    `.sidebar-nav a[data-view="${viewName}"]`,
  );
...
export function renderSystem() {
  ...
  if (userNameEl && state.perfil?.nome)
    userNameEl.textContent = state.perfil.nome;

  // STATS
  const statEstudos = document.querySelector("#stat-estudos .stat-value");
  if (statEstudos)
    statEstudos.textContent = `${state.estudos?.horasHoje || 0} h`;

  const statProjetos = document.querySelector("#stat-projetos .stat-value");
  if (statProjetos)
    statProjetos.textContent = Array.isArray(state.projetos)
  ...
  if (perfilEl)
    perfilEl.innerHTML = renderPerfilPage(state.perfil, state.systemStatus);
  ...
  document.addEventListener("click", async (e) => {
    ...
    if (target.closest("#btn-select-foco")) {
      ...
      if (projetos.length === 0) {
        await customModal({
          title: "Aviso",
          message:
            'Você ainda não tem projetos cadastrados! Crie um na aba "Projetos" primeiro.',
          isConfirm: true,
        });
        return;
      }
      ...
      if (escolha !== null && escolha !== undefined) {
        const proj = projetos.find((p) => p.id === Number(escolha));
        if (proj) {
    ...
    if (target.closest("#btn-add-materia")) {
      ...
      const progressoNum = Math.min(
        100,
        Math.max(0, Number(progressoInput) || 0),
      );
      ...
      const payloadBackend = {
        ...
        metaHorasSemanal:
          Number(
            state.estudos.metaHorasSemanal || state.estudos.metasHorasSemanal,
          ) || 0,
        ultimaAtualizacao: new Date().toISOString(),
        ultimoReset: state.estudos.ultimoReset,
        ultimoResetSemanal: state.estudos.ultimoResetSemanal,
    ...
    if (
      target.closest("#stat-status") ||
      target.closest("#btn-change-status") ||
      target.closest("#user-profile-status")
    ) {
      const novoStatus = await customModal({
