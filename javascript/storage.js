...
export async function loadSystemData() {
  ...
  const state = {
    ...
    projetos: Array.isArray(projetosAPI)
      ? projetosAPI
      : localData.projetos || projetosIniciais,
    estudos: estudosFinais,
    metas: Array.isArray(metasAPI)
      ? metasAPI
      : localData.metas || metasIniciais,
    lazer: lazerFinal,
...
export async function salvarPerfilAPI(perfil) {
  if (!perfil?.id) {
    console.error(
      'salvarPerfilAPI: perfil sem "id" — recarregue o estado antes de editar.',
    );
    return null;
  }
